...
`EMBEDDING_MODEL_CONFIG__MAX_BATCH_SIZE` defaults to the native provider limit
(2048 for OpenAI, 100 for Gemini). Set it when an OpenAI-compatible embedding
provider accepts fewer inputs per request, such as DashScope `text-embedding-v4`
with a limit of 10.
